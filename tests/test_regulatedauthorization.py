...
from fhir.resources import regulatedauthorization
...
def impl_regulatedauthorization_1(inst):
    ...
    assert (
        inst.jurisdictionalAuthorization[0].reference
        == "RegulatedAuthorization/Jurusdictional1"
    )
...
def impl_regulatedauthorization_2(inst):
    ...
    assert inst.subject.reference == "MedicinalProductDefinition/equilidem"
    assert inst.text.status == "generated"
