...
from fhir.resources import requirements
...
def test_requirements_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "Requirements-example2.json"
    )
...
def impl_requirements_2(inst):
    ...
    assert inst.statement[0].requirement == "The server SHALL support expanding value sets"
    assert inst.statement[0].satisfiedBy[0] == "http://hl7.org/fhir/terminology-service.html#expand"
...
def test_requirements_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "Requirements-example1.json"
    )
    inst = requirements.Requirements.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
