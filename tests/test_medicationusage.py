...
def test_medicationusage_1(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample2.json"
...
def test_medicationusage_2(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample3.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
...
def test_medicationusage_3(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample1.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
...
def impl_medicationusage_4(inst):
    assert inst.adherence.coding[0].code == "not-taking"
    assert (
        inst.adherence.coding[0].system
        == "http://hl7.org/fhir/CodeSystem/medication-usage-adherence"
    )
    assert inst.dateAsserted == fhirtypes.DateTime.validate("2015-02-22")
    assert inst.dosage[0].asNeededBoolean is False
    assert inst.dosage[0].maxDosePerPeriod.denominator.code == "d"
    ...
    assert inst.reason[0].concept.coding[0].code == "65363002"
    assert inst.reason[0].concept.coding[0].display == "Otitis Media"
    assert inst.reason[0].concept.coding[0].system == "http://snomed.info/sct"
    assert inst.status == "completed"
    assert inst.subject.display == "Donald Duck"
    assert inst.subject.reference == "Patient/pat1"
...
def test_medicationusage_4(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample4.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
...
def test_medicationusage_5(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample5.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
...
def test_medicationusage_6(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample6.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
...
def test_medicationusage_7(base_settings):
    ...
    """
    filename = base_settings["unittest_data_dir"] / "medicationusageexample7.json"
    inst = medicationusage.MedicationUsage.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
    assert "MedicationUsage" == inst.resource_type
