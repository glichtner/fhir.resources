...
def impl_group_5(inst):
    ...
    assert inst.identifier[0].system == "http://www.somesystemabc.net/identifiers/groups"
    ...
    assert inst.member[0].period.start == fhirtypes.DateTime.validate("2021-01-01T01:01:10-06:00")
    assert inst.member[1].entity.reference == "RelatedPerson/relatedPersonDenovoMother"
    assert inst.member[1].inactive is False
    assert inst.member[1].period.start == fhirtypes.DateTime.validate("2021-01-01T01:01:10-06:00")
    assert inst.member[2].entity.reference == "RelatedPerson/relatedPersonDenovoFather"
    assert inst.member[2].inactive is False
    assert inst.member[2].period.start == fhirtypes.DateTime.validate("2021-01-01T01:01:10-06:00")
    assert inst.membership == "enumerated"
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
...
def test_group_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "Group-denovoFamily.json"
    )
    inst = group.Group.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
