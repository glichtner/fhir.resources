...
from fhir.resources import genomicstudy
...
def impl_genomicstudy_1(inst):
    ...
    assert inst.analysis[0].changeType[0].coding[0].system == "http://sequenceontology.org"
    assert inst.analysis[0].changeType[1].coding[0].code == "SO:0002007"
    assert inst.analysis[0].changeType[1].coding[0].display == "MNV"
    assert inst.analysis[0].changeType[1].coding[0].system == "http://sequenceontology.org"
    assert inst.analysis[0].changeType[2].coding[0].code == "SO:1000032"
    assert inst.analysis[0].changeType[2].coding[0].display == "delins"
    assert inst.analysis[0].changeType[2].coding[0].system == "http://sequenceontology.org"
    assert inst.analysis[0].date == fhirtypes.DateTime.validate("2019-03-01T01:01:10-06:00")
    assert inst.analysis[0].device[0].device.reference == "Device/NGS-device"
    assert inst.analysis[0].device[0].function.coding[0].display == "Next Generation Sequencing"
    ...
    assert inst.analysis[0].methodType[0].coding[0].display == "Nucleic acid sequencing (procedure)"
    assert inst.analysis[0].methodType[0].coding[0].system == "http://snomed.info/sct"
    assert inst.analysis[0].note[0].text == (
    "For technical reasons, PIK3CB was deemed uncallable using "
    "this method."
    )
    assert inst.analysis[0].output[0].file.reference == "DocumentReference/genomicVCFfile_simple"
    assert inst.analysis[0].output[0].type.coding[0].code == "vcf"
    assert inst.analysis[0].output[0].type.coding[0].display == "VCF"
    assert inst.analysis[0].performer[0].actor.reference == "Practitioner/practitioner02"
    ...
    assert inst.analysis[1].changeType[0].coding[0].system == "http://sequenceontology.org"
    assert inst.analysis[1].date == fhirtypes.DateTime.validate("2019-03-01T01:01:10-06:00")
    ...
    assert inst.analysis[1].methodType[0].coding[0].display == "Nucleic acid sequencing (procedure)"
    assert inst.analysis[1].methodType[0].coding[0].system == "http://snomed.info/sct"
    assert inst.analysis[1].note[0].text == (
    "For technical reasons, PIK3CB was deemed uncallable using "
    "this method."
    )
    assert inst.analysis[1].output[0].file.reference == "DocumentReference/genomicVCFfile_cnv"
    assert inst.analysis[1].output[0].type.coding[0].code == "vcf"
    assert inst.analysis[1].output[0].type.coding[0].display == "VCF"
    assert inst.analysis[1].performer[0].actor.reference == "Practitioner/practitioner02"
    ...
    assert inst.identifier[0].system == "http://www.somesystemabc.net/identifiers/genomicstudies"
...
def test_genomicstudy_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "genomicstudy-example-lungMass.json"
    )
...
def impl_genomicstudy_2(inst):
    assert inst.analysis[0].date == fhirtypes.DateTime.validate("2021-01-01T01:01:10-06:00")
    assert inst.analysis[0].device[0].device.reference == "Device/NGS-device"
    assert inst.analysis[0].device[0].function.coding[0].display == "Next Generation Sequencing"
    assert inst.analysis[0].identifier[0].system == "http://www.somesystemabc.net/identifiers/genomicAnalyses"
    ...
    assert inst.analysis[0].instantiatesUri == "https://pubmed.ncbi.nlm.nih.gov/33927380/"
    assert inst.analysis[0].note[0].text == "This is a next generation sequencing analysis of a proband."
    assert inst.analysis[0].performer[0].actor.reference == "Practitioner/practitioner02"
    assert inst.analysis[0].performer[0].role.coding[0].code == "PRF"
    assert inst.analysis[0].performer[0].role.coding[0].display == "Performer"
    assert inst.analysis[0].performer[0].role.coding[0].system == (
    ...
    assert inst.analysis[1].instantiatesUri == "https://pubmed.ncbi.nlm.nih.gov/33927380/"
    assert inst.analysis[1].note[0].text == (
    "This is a next generation sequencing analysis of a mother of"
    " a proband."
    )
    assert inst.analysis[1].performer[0].actor.reference == "Practitioner/practitioner02"
    assert inst.analysis[1].performer[0].role.coding[0].code == "PRF"
    ...
    assert inst.analysis[2].instantiatesUri == "https://pubmed.ncbi.nlm.nih.gov/33927380/"
    assert inst.analysis[2].note[0].text == (
    "This is a next generation sequencing analysis of a father of"
    " a proband."
    )
    assert inst.analysis[2].performer[0].actor.reference == "Practitioner/practitioner02"
    ...
    assert inst.analysis[3].date == fhirtypes.DateTime.validate("2021-01-01T03:01:10-06:00")
    assert inst.analysis[3].device[0].device.reference == "Device/Triodenovo-SW"
    assert inst.analysis[3].identifier[0].system == "http://www.somesystemabc.net/identifiers/genomicAnalyses"
    ...
    assert inst.analysis[3].instantiatesUri == "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6885382/"
    ...
    assert inst.analysis[3].performer[0].actor.reference == "Practitioner/practitioner02"
    ...
    assert inst.reason[0].concept.coding[0].display == "Disorder of lipid metabolism (disorder)"
    assert inst.reason[0].concept.coding[0].system == "http://snomed.info/sct"
    assert inst.referrer.reference == "Practitioner/practitioner01"
...
def test_genomicstudy_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "genomicstudy-example.json"
    )
    inst = genomicstudy.GenomicStudy.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_genomicstudy_3(inst):
    ...
    assert inst.analysis[2].input[0].file.reference == "DocumentReference/genomicFileProband"
    assert inst.analysis[2].input[0].type.coding[0].code == "bam"
    assert inst.analysis[2].input[0].type.coding[0].display == "BAM"
    assert inst.analysis[2].input[0].type.coding[0].system == "http://hl7.org/fhir/genomicstudy-dataformat"
    assert inst.analysis[2].input[1].file.reference == "DocumentReference/genomicFileMother"
    assert inst.analysis[2].input[1].type.coding[0].code == "bam"
    assert inst.analysis[2].input[1].type.coding[0].display == "BAM"
    assert inst.analysis[2].input[1].type.coding[0].system == "http://hl7.org/fhir/genomicstudy-dataformat"
    assert inst.analysis[2].input[2].file.reference == "DocumentReference/genomicFileFather"
    assert inst.analysis[2].input[2].type.coding[0].code == "bam"
    assert inst.analysis[2].input[2].type.coding[0].display == "BAM"
    assert inst.analysis[2].input[2].type.coding[0].system == "http://hl7.org/fhir/genomicstudy-dataformat"
    assert inst.analysis[2].instantiatesUri == "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6885382/"
    assert inst.analysis[2].note[0].text == (
    "This is a next generation sequencing analysis of the "
    "comparison analysis of proband and parents sequences."
    )
    assert inst.analysis[2].output[0].file.reference == "DocumentReference/genomicFileGroupAsSubject"
    assert inst.analysis[2].output[0].type.coding[0].code == "vcf"
    assert inst.analysis[2].output[0].type.coding[0].display == "VCF"
    assert inst.analysis[2].output[0].type.coding[0].system == "http://hl7.org/fhir/genomicstudy-dataformat"
    assert inst.analysis[2].performer[0].actor.reference == "Practitioner/practitioner02"
    assert inst.analysis[2].performer[0].role.coding[0].code == "PRF"
    assert inst.analysis[2].performer[0].role.coding[0].display == "Performer"
    assert inst.analysis[2].performer[0].role.coding[0].system == (
    ...
    assert inst.reason[0].concept.coding[0].display == "Cystic fibrosis, prenatal detection (procedure) |"
    assert inst.reason[0].concept.coding[0].system == "http://snomed.info/sct"
    assert inst.referrer.reference == "Practitioner/practitioner01"
...
def test_genomicstudy_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "genomicstudy-example-trio2.json"
    )
    inst = genomicstudy.GenomicStudy.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
