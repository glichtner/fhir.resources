...
def impl_imagingstudy_1(inst):
    ...
    assert inst.procedure[0].valueReference.reference == "Procedure/example"
    assert inst.procedure[1].valueCodeableConcept.coding[0].code == "RPID2589"
    assert (
        inst.procedure[1].valueCodeableConcept.coding[0].display == "XR Wrist 3+ Views"
    )
    assert (
        inst.procedure[1].valueCodeableConcept.coding[0].system
        == "http://www.radlex.org"
    )
    assert inst.procedure[1].valueCodeableConcept.text == "XR Wrist 3+ Views"
