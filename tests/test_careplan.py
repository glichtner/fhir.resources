...
def test_careplan_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "careplan-example-f203-sepsis.json"
    )
    inst = careplan.CarePlan.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_careplan_6(inst):
    assert inst.activity[0].plannedActivityReference.display == "Prenatal vitamin MedicationRequest"
