...
def impl_medicationadministration_1(inst):
    assert inst.dosage.dose.code == "PATCH"
    assert inst.dosage.dose.system == "http://terminology.hl7.org/CodeSystem/v3-orderableDrugForm"
    assert inst.dosage.dose.unit == "patch"
    assert float(inst.dosage.dose.value) == float(1)
    assert inst.dosage.text == "apply one patch three times per week"
    assert inst.id == "medadmin0312"
    assert inst.identifier[0].system == "http://www.bmc.nl/portal/administrations"
...
def test_medicationadministration_1(base_settings):
    ...
    Test File: medicationadministration0312.json
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationadministration0312.json"
    )
...
def test_medicationadministration_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationadministration0303.json"
    )
    inst = medicationadministration.MedicationAdministration.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationadministration_6(inst):
    ...
    assert inst.medication.concept.coding[0].display == "Paracetamol 240mg suppository (product)"
    assert inst.medication.concept.coding[0].system == "http://snomed.info/sct"
    assert inst.meta.tag[0].code == "HTEST"
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
...
def impl_medicationadministration_7(inst):
    ...
    "http://terminology.hl7.org/CodeSystem/medication-admin-"
    "category"
    )
...
def test_medicationadministration_7(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationadministration0302.json"
    )
    inst = medicationadministration.MedicationAdministration.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationadministration_8(inst):
    ...
    assert inst.occurencePeriod.end == fhirtypes.DateTime.validate("2015-01-15T14:30:00+01:00")
    assert inst.occurencePeriod.start == fhirtypes.DateTime.validate("2015-01-15T14:30:00+01:00")
