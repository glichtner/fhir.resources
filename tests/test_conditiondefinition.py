...
def impl_conditiondefinition_1(inst):
    ...
    assert inst.medication[0].code.coding[0].system == "http://www.nlm.nih.gov/research/umls/rxnorm"
    assert inst.name == "Hyperlipidemia"
    assert inst.observation[0].code.coding[0].code == "24331-1"
    assert inst.observation[0].code.coding[0].display == "Lipid 1996 panel - Serum or Plasma"
    ...
    "Cholesterol in LDL [Mass/volume] in Serum or Plasma by "
    "calculation"
    )
    ...
    "http://acme.com/fhir/PlanDefinition/fd0e32d8-2ebd-4211-9e50-"
    "8983efddc2c6"
    )
...
def test_conditiondefinition_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "conditiondefinition-example.json"
    )
