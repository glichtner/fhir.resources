...
def impl_medicationstatement_2(inst):
    ...
    "http://hl7.org/fhir/CodeSystem/medication-statement-"
    "adherence"
    )
...
def impl_medicationstatement_5(inst):
    ...
    assert inst.meta.tag[0].display == "test health data"
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.note[0].text == "Patient reports they used the medication patch on this day."
...
def test_medicationstatement_5(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationstatementexample9.json"
    )
    inst = medicationstatement.MedicationStatement.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_medicationstatement_9(inst):
    ...
    assert inst.meta.tag[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.note[0].text == (
    "Patient reports they used the given patches over the last 30"
    " days"
    )
    assert inst.status == "recorded"
    assert inst.subject.display == "Donald Duck"
...
def test_medicationstatement_9(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "medicationstatementexample8.json"
    )
    inst = medicationstatement.MedicationStatement.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
