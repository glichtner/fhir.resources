...
def impl_nutritionorder_4(inst):
    ...
    assert inst.id == "energysupplement"
    assert (
        inst.identifier[0].system == "http://goodhealthhospital.org/nutrition-requests"
    )
    assert inst.identifier[0].value == "123"
    ...
    assert inst.patient.reference == "Patient/example"
    assert inst.status == "active"
    assert inst.supplement[0].productName == "Ensure"
    assert inst.supplement[0].quantity.unit == "8 oz can"
    assert float(inst.supplement[0].quantity.value) == float(1)
    assert inst.supplement[0].schedule[
        0
    ].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2015-02-10")
    assert inst.supplement[0].schedule[0].repeat.frequency == 3
    assert float(inst.supplement[0].schedule[0].repeat.period) == float(1)
    assert inst.supplement[0].schedule[0].repeat.periodUnit == "d"
    assert inst.supplement[0].type.coding[0].code == "442971000124100"
    assert inst.supplement[0].type.coding[0].display == "Adult high energy formula"
    assert inst.supplement[0].type.coding[0].system == "http://snomed.info/sct"
    assert inst.supplement[0].type.coding[1].code == "1010"
    assert inst.supplement[0].type.coding[1].display == "Adult high energy drink"
    assert (
        inst.supplement[0].type.coding[1].system
        == "http://goodhealthhospital.org/supplement-type-codes"
    )
    assert inst.supplement[0].type.text == "Adult high energy drink"
    assert inst.text.status == "generated"


def test_nutritionorder_4(base_settings):
    """No. 4 tests collection for NutritionOrder.
    Test File: nutritionorder-example-energysupplement.json
    """
    filename = (
        base_settings["unittest_data_dir"]
        / "nutritionorder-example-energysupplement.json"
    )
    inst = nutritionorder.NutritionOrder.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_nutritionorder_6(inst):
    ...
    assert inst.supplement[0].schedule[0].repeat.when[0] == "C"
    assert inst.supplement[0].type.coding[0].code == "442991000124104"
    assert inst.supplement[0].type.coding[0].display == "Adult high protein formula"
    assert inst.supplement[0].type.coding[0].system == "http://snomed.info/sct"
    assert inst.supplement[0].type.coding[1].code == "1000"
    assert inst.supplement[0].type.coding[1].display == "High Protein Powder"
    assert (
        inst.supplement[0].type.coding[1].system
        == "http://goodhealthhospital.org/supplement-type-codes"
    )
    assert inst.supplement[0].type.text == "High Protein Powder"
    assert inst.text.status == "generated"
...
def test_nutritionorder_6(base_settings):
    ...
    filename = (
        base_settings["unittest_data_dir"]
        / "nutritionorder-example-proteinsupplement.json"
    )
    inst = nutritionorder.NutritionOrder.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_nutritionorder_10(inst):
    ...
    assert (
        inst.oralDiet.nutrient[2].modifier.coding[0].system == "http://snomed.info/sct"
    )
    assert inst.oralDiet.schedule[
        0
    ].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2015-02-10")
