...
def impl_nutritionorder_1(inst):
    ...
    assert inst.oralDiet.nutrient[1].amount.code == "mg"
    assert inst.oralDiet.nutrient[1].amount.system == "http://unitsofmeasure.org"
    assert inst.oralDiet.nutrient[1].amount.unit == "milligrams"
    assert float(inst.oralDiet.nutrient[1].amount.value) == float(1000)
    assert inst.oralDiet.nutrient[1].modifier.coding[0].code == "30820000"
    assert inst.oralDiet.nutrient[1].modifier.coding[0].display == "Phosphorus"
    assert inst.oralDiet.nutrient[1].modifier.coding[0].system == "http://snomed.info/sct"
    assert inst.oralDiet.nutrient[2].amount.code == "g"
    assert inst.oralDiet.nutrient[2].amount.system == "http://unitsofmeasure.org"
    assert inst.oralDiet.nutrient[2].amount.unit == "grams"
    assert float(inst.oralDiet.nutrient[2].amount.value) == float(2)
    assert inst.oralDiet.nutrient[2].modifier.coding[0].code == "88480006"
    assert inst.oralDiet.nutrient[2].modifier.coding[0].display == "Potassium"
    assert inst.oralDiet.nutrient[2].modifier.coding[0].system == "http://snomed.info/sct"
    assert inst.oralDiet.schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2015-02-10")
    ...
    assert inst.oralDiet.type[0].coding[0].code == "33489005"
    assert inst.oralDiet.type[0].coding[0].display == "Renal diet"
    assert inst.oralDiet.type[0].coding[0].system == "http://snomed.info/sct"
    assert inst.oralDiet.type[0].coding[1].code == "1050"
    assert inst.oralDiet.type[0].coding[1].display == "Renal"
    assert inst.oralDiet.type[0].coding[1].system == "http://goodhealthhospital.org/diet-type-codes"
    assert inst.oralDiet.type[0].text == "Renal Diet"
...
def test_nutritionorder_1(base_settings):
    ...
    Test File: nutritionorder-example-renaldiet.json
    """
    filename = (
        base_settings["unittest_data_dir"] / "nutritionorder-example-renaldiet.json"
    )
...
def impl_nutritionorder_4(inst):
    ...
    assert inst.enteralFormula.additive[0].type.concept.coding[0].system == "http://terminology.hl7.org/CodeSystem/entformula-additive"
    ...
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2014-09-17T16:00:00Z")
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.frequency == 1
    assert float(inst.enteralFormula.administration[0].schedule.timing[0].repeat.period) == float(4)
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.periodUnit == "h"
    assert inst.enteralFormula.baseFormulaProductName == "Acme High Protein Formula"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].code == "442991000124104"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].display == "Adult high protein formula"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].system == "http://snomed.info/sct"
...
def impl_nutritionorder_7(inst):
    ...
    assert inst.encounter.display == "Inpatient"
    assert inst.encounter.reference == "Encounter/example"
    assert inst.id == "proteinsupplement"
    assert inst.identifier[0].system == "http://goodhealthhospital.org/nutrition-requests"
    assert inst.identifier[0].value == "123"
    assert inst.intent == "order"
    assert inst.meta.tag[0].code == "HTEST"
    ...
    assert inst.subject.display == "Peter Chalmers"
    assert inst.subject.reference == "Patient/example"
    assert inst.supplement[0].instruction == "Beneprotein 1 scoop TID with meal.starting on  2015-02-10"
    assert inst.supplement[0].productName == "Beneprotein"
    assert inst.supplement[0].quantity.code == "{scoop}"
    assert inst.supplement[0].quantity.system == "http://unitsofmeasure.org"
    assert inst.supplement[0].quantity.unit == "scoop"
    assert float(inst.supplement[0].quantity.value) == float(1)
    assert inst.supplement[0].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2015-02-10")
    assert inst.supplement[0].schedule.timing[0].repeat.when[0] == "C"
    assert inst.supplement[0].type.concept.coding[0].code == "442991000124104"
    assert inst.supplement[0].type.concept.coding[0].display == "Adult high protein formula"
    assert inst.supplement[0].type.concept.coding[0].system == "http://snomed.info/sct"
    assert inst.supplement[0].type.concept.coding[1].code == "1000"
    assert inst.supplement[0].type.concept.coding[1].display == "High Protein Powder"
    assert inst.supplement[0].type.concept.coding[1].system == "http://goodhealthhospital.org/supplement-type-codes"
    assert inst.supplement[0].type.concept.text == "High Protein Powder"
    assert inst.text.status == "generated"


def test_nutritionorder_7(base_settings):
    """No. 7 tests collection for NutritionOrder.
    Test File: nutritionorder-example-proteinsupplement.json
    """
    filename = (
        base_settings["unittest_data_dir"] / "nutritionorder-example-proteinsupplement.json"
    )
    inst = nutritionorder.NutritionOrder.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def impl_nutritionorder_8(inst):
    ...
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2014-09-17T07:00:00Z")
    ...
    assert inst.enteralFormula.administration[1].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2014-09-17T11:00:00Z")
    ...
    assert inst.enteralFormula.administration[2].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2014-09-17T15:00:00Z")
    assert inst.enteralFormula.baseFormulaProductName == " Acme Diabetes Formula"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].code == "6547210000124112"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].display == "Diabetic specialty enteral formula"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].system == "http://snomed.info/sct"
    assert inst.enteralFormula.caloricDensity.code == "cal/mL"
    assert inst.enteralFormula.caloricDensity.system == "http://unitsofmeasure.org"
    assert inst.enteralFormula.caloricDensity.unit == "calories per milliliter"
...
def impl_nutritionorder_9(inst):
    ...
    assert inst.supplement[0].type.concept.coding[0].display == "Adult high energy formula"
    assert inst.supplement[0].type.concept.coding[0].system == "http://snomed.info/sct"
    assert inst.supplement[0].type.concept.coding[1].code == "1040"
    assert inst.supplement[0].type.concept.coding[1].display == "Adult high energy pudding"
    assert inst.supplement[0].type.concept.coding[1].system == "http://goodhealthhospital.org/supplement-type-codes"
...
def impl_nutritionorder_10(inst):
    ...
    assert inst.encounter.display == "Inpatient"
    assert inst.encounter.reference == "Encounter/example"
    assert inst.enteralFormula.additive[0].productName == "Acme High Carbohydrate Additive"
    assert inst.enteralFormula.additive[0].type.concept.coding[0].code == "carbohydrate"
    assert inst.enteralFormula.additive[0].type.concept.coding[0].display == "Carbohydrate"
    assert inst.enteralFormula.additive[0].type.concept.coding[0].system == "http://terminology.hl7.org/CodeSystem/entformula-additive"
    assert inst.enteralFormula.administrationInstruction == (
    ...
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.boundsPeriod.start == fhirtypes.DateTime.validate("2014-09-17")
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.frequency == 1
    assert float(inst.enteralFormula.administration[0].schedule.timing[0].repeat.period) == float(3)
    assert inst.enteralFormula.administration[0].schedule.timing[0].repeat.periodUnit == "h"
    ...
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].display == "infant formula + iron"
    assert inst.enteralFormula.baseFormulaType.concept.coding[0].system == "http://snomed.info/sct"
