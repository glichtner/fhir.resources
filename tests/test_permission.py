...
def test_permission_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "permission-example.json"
    )
...
def impl_permission_2(inst):
    ...
    assert inst.rule[0].activity[0].action[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/consentaction"
    ...
    assert inst.rule[0].activity[0].purpose[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActReason"
    assert inst.rule[0].data[0].expression.expression == (
    "http://hl7.org/fhir/uv/saner/Measure/CDCHealthcareSupplyPath"
    "way"
    )
    assert inst.rule[0].data[0].expression.language == "text/fhirpath"
    assert inst.rule[0].limit[0].coding[0].code == "AUDIT"
    assert inst.rule[0].limit[0].coding[0].system == "http://terminology.hl7.org/CodeSystem/v3-ActCode"
...
def test_permission_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "permission-example-saner.json"
    )
    inst = permission.Permission.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
...
def test_permission_3(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "permission-example-vhdir.json"
    )
    inst = permission.Permission.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
