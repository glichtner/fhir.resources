...
def test_task_1(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-cpg-example-1.json"
    )
...
def impl_task_2(inst):
    ...
    assert inst.executionPeriod.end == fhirtypes.DateTime.validate("2016-10-31T14:45:05+10:00")
    ...
    assert inst.requestedPerformer[0].concept.coding[0].display == "Laboratory hematologist"
    assert inst.requestedPerformer[0].concept.coding[0].system == "http://snomed.info/sct"
...
def test_task_2(base_settings):
    ...
    """
    filename = (
        base_settings["unittest_data_dir"] / "task-example4.json"
    )
    inst = task.Task.parse_file(
        filename, content_type="application/json", encoding="utf-8"
    )
