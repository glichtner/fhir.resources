...
from fhir.resources import medicinalproductdefinition
...
def impl_medicinalproductdefinition_1(inst):
    ...
    assert inst.contained[3].id == "ingredient"
    ...
    assert inst.package[0].package.reference == "#package"
...
def impl_medicinalproductdefinition_2(inst):
    ...
    assert inst.contained[2].id == "Acetamin-ing-500"
    ...
    assert inst.package[0].package.reference == "#Acetamin-pack-20"
    assert inst.text.status == "generated"
...
def impl_medicinalproductdefinition_3(inst):
    ...
    assert inst.ingredient[0].reference.display == "Equilidonium Phosphate"
    assert inst.ingredient[1].reference.display == "Calcium Carbonate"
...
def impl_medicinalproductdefinition_4(inst):
    assert inst.administrableProduct[0].product.reference == "#administrable-form"
    assert (
    ...
    assert inst.contained[4].id == "ingredientPowder"
    assert inst.contained[5].id == "ingredientSolvent"
    assert inst.contained[6].id == "administrable-form"
...
def impl_medicinalproductdefinition_5(inst):
    ...
    assert inst.identifier[0].system == "http://example.org.uk/fhir/product"
    assert inst.identifier[0].value == "Equilidem25"
    assert inst.ingredient[0].reference.reference == "#EquilidoniumPhosphate"
    assert inst.ingredient[1].reference.reference == "#CalciumCarbonate"
    assert inst.legalStatusOfSupply.coding[0].code == "POM"
    assert inst.legalStatusOfSupply.coding[0].display == "Prescription only medicine"
    assert (
        inst.legalStatusOfSupply.coding[0].system
...
def impl_medicinalproductdefinition_6(inst):
    ...
    assert inst.name[0].productName == "Equilidem 2.5 mg film-coated tablets"
    assert inst.operation[0].authorization.identifier.system == (
        "http://ema.europa.eu/example/manufacturingAuthorisationRefer" "enceNumber"
    )
    assert inst.operation[0].authorization.identifier.value == "1324TZ"
...
def impl_medicinalproductdefinition_7(inst):
    ...
    assert inst.package[0].package.reference == "#packageCombo"
    assert inst.text.status == "generated"
